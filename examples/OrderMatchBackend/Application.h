...
class Application : public FIX::Application, public FIX::MessageCracker {
  ...
  void processCancel(const std::string &id, const std::string &symbol,
                     Order::Side);
  void processReplace(const std::string &origId, const std::string &newId,
                      const std::string &symbol, Order::Side side, double price,
                      long qty);

  void updateOrder(const Order &, char status, const std::string &origId = {});
  ...
  void fillOrder(const Order &order) {
    updateOrder(order, order.isFilled() ? FIX::OrdStatus_FILLED
  ...
  std::set<FIX::SessionID> m_activeSessions;
